...
def get_personalized_recommendations(user_id, limit=8):
    ...
    # We look at both borrowed and saved books
    ...
    # 4. Fallback/Trending: If not enough recommendations, fill with top-rated books
    if len(recommendations) < limit:
        needed = limit - len(recommendations)
        existing_ids = [b.id for b in recommendations] + borrowed_ids
        
        # Get books with highest number of copies (proxy for popular) or just newest
